#!/bin/bash
# Build script for inventory service
set -e
...
IMAGE_NAME="${IMAGE_NAME:-inventory-service}"
...
Arguments:
  image:tag         Image name and tag (default: inventory-service:latest)
...
Environment Variables:
  IMAGE_NAME        Image name (default: inventory-service)
...
Examples:
  ...
  $0 inventory-service:v1.0.0

  # Build and push to Docker Hub
  $0 myuser/inventory-service:v1.0.0 --push
...
log_info "Next steps:"
if [ "$PUSH" = true ]; then
    log_info "  Deploy with: kubectl apply -f scenarios/<job>.yaml"
    log_info "  Or via Helm: helm install inventory-service ./helm/inventory-service -f values-batch-reporting.yaml"
else
    log_info "  Run locally: docker run --rm -e MODE=batch -e BATCH_JOBS=report-generation ... $FULL_IMAGE"
